// src/components/Tour/HomeFeedTourTooltip.tsx
...
const HomeFeedTourTooltip: React.FC<HomeFeedTourTooltipProps> = ({
  ...
  useEffect(() => {
    ...
    const calculatePosition = () => {
      ...
      // Special handling for feed header (step 0)
      if (currentStep === 0) {
        // For header, position BELOW
        tooltipTop = targetRect.bottom + 20;
        arrowPosition = 'top';
      } 
      // For create call-to-action (last step)
      else if (currentStep === totalSteps - 1) {
        // Position ABOVE with extra spacing
        tooltipTop = targetRect.top - TOOLTIP_HEIGHT - 15;
        arrowPosition = 'bottom';
      }
      // For card items (steps 1-4)
      else if (currentStep >= 1 && currentStep <= 4) {
        // Try to position below if space, otherwise above
  ...
  }, [targetElement, currentStep, totalSteps]);
  ...
  // Get target element position for highlight ring
  const targetRect = targetElement.getBoundingClientRect();
  ...
  return (
    <>
      ...
      >
        {/* Arrow pointing to target - FIXED: No duplicate properties */}
        <div
          style={{
            position: 'absolute',
