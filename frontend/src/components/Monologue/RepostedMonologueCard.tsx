...
interface RepostedMonologueCardProps {
  repost: {
    id: string;
    user_id: string;
    user_name: string;
  ...
  originalMonologue: {
    ...
    user_name: string;
    user_genre_tag: string;
    profiles?: { // ← UPDATED: Match MonologueCard structure
      avatar_url?: string;
    };
    title: string;
    content_text: string;
...
// SVG Icons for repost engagement (same as MonologueCard but can be customized)
...
const RepostedMonologueCard: React.FC<RepostedMonologueCardProps> = React.memo(({
  repost,
  originalMonologue,
  currentUserId,
  onAction
  ...
  // ADDED: Use monologue hook for deleteRepost functionality
  const { deleteRepost } = useMonologue();
  
  // UPDATED: All hooks now use 'monologue_repost' and repost.id for separate engagement
  const likeMutation = useLikeItem();
  const { data: likesData } = useLikesStatus({ 
    content_type: 'monologue_repost', 
  ...
  const handleUndoRepost = useCallback(async () => {
    ...
    try {
      ...
      if (success) {
        onAction?.('unrepost', repost.id, { originalMonologueId: originalMonologue.id });
      }
    } catch (error) {
  ...
  }, [repost.id, originalMonologue.id, currentUserId, deleteRepost, onAction]);

  // UPDATED: Real like handler for REPOST (not original monologue)
  ...
  const handleEmbeddedCardClick = () => {
    // Navigate to original monologue - FIXED: Add context with originalMonologueId
  ...
  return (
    <article 
      ...
      aria-label={`Reposted monologue by ${repost.user_name}`}
      role="article"
    >
      ...
      {showShareDialog && (
        <ShareDialog
          ...
          shareUrl={`${window.location.origin}/monologue_repost/${repost.id}`}
          // ADDED: Normalized content and creator data for social sharing
          content={{
            title: originalMonologue.title,
            excerpt: originalMonologue.content_text || originalMonologue.title,
            images: [] // Monologues typically don't have images
          }}
          creator={{
            name: originalMonologue.user_name,
...
RepostedMonologueCard.displayName = 'RepostedMonologueCard';

export default RepostedMonologueCard;